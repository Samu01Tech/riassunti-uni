...
> User authenticates through the IdP which resposnds with the full user identity object. It is then saved on the SP identity store. The linking is done out of the SSO procedure. 

### Federation via Persistent Pseudonym Identifiers

SAML 2.0 has the ability to dynamically estabilish a federated identity for a user as part ow the Web SSO message exchanage. Basically a user is given a common Id that is the same across the federation but data might be different (ie. different username). If the SP does not have the user in the store than it creates it with the id given by the IP.
